...
# Codes are copied from https://github.com/coqui-ai/TTS/blob/dev/TTS/tts/utils/helpers.py
# some declarations are deleted.
...
try:
    from utils_.monotonic_align.core import maximum_path_c
    CYTHON = True
...
def sequence_mask(sequence_length, max_len=None):
    ...
    Args:
        sequence_length (torch.tensor): Sequence lengths. - shape : [B]
...
def segment(x: torch.tensor, segment_indices: torch.tensor, segment_size=4, pad_short=False):
    ...
    Args:
        x (torch.tensor): Input tensor. ## x.size(-1) == time axis (mo) - [B, feature_dim , time]
