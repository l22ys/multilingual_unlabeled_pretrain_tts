...
class Trainer:
    ...
    def train_on_one_gpu(rank, world_size, self):
        ...
        for key in self.conf.for_wandb.config.keys(): 
            wandb_config[key] = self.conf.for_wandb.config[key] 
    ...
    def train_epoch(self):
        ...
        for _ in pbar_step:
            ...
            assert self.conf.logging.wandb_freq > 1
    ...
    def train_step(self, batch: Dict)-> Tuple[Optional[Dict],Optional[Dict]]:
        ...
        for key, value in batch.items():
            if isinstance(value, torch.Tensor):
                ...
                batch[key] = value.to('cuda:{}'.format(self.rank))
        ...
        loss_data['discriminator'] = self.losses['Dis'](scores_disc_real, scores_disc_fake) # dict
    ...
    def save(self, epoch_eval_loss):
        ...
        if (not self.is_ddp) or (self.is_ddp and self.rank == 0):
            ...
            # in case there's no sufficient storage
