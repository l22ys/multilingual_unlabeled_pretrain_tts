# Codes are copied and modified from https://github.com/dhchoi99/NANSY/blob/master/models/ecapa.py
...
class Speaker(torch.nn.Module):
    def __init__(self, c_in=1024, c_mid=512, c_out=192):
        ...
        self.spk = ECAPA_TDNN(c_in=c_in, c_mid=c_mid, c_out=c_out)
    ...
    def forward(self, x):
        ...
        """        
        y = self.spk(x)  # B x C(1024) x t -> B x D(192)
